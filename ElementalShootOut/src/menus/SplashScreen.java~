...
/**
 * The SplashScreen class creates the a splash screen, which displays the game logo, game name, company name, company logo, 
 * and the developer names within a window. After a set number of seconds, the window disposes itself, and creates the game 
 * window.
 * 
 * @author Anqi Wu
...
{
  /**
   * firstRun - protected static boolean - Stores whether this is the first run.
   */
  protected static boolean firstRun;
  
  /**
   * Constructs an instance of SplashScreen with the image set by the string parameter and the time in
   * milliseconds before the splash screen closes set by the int parameter. The window is centered on the screen. This
   * constructor also adds teo anonymous classes that implement Runnable and another that extends MouseAdapter. 
   * The first, closerRunner, closes the splash screen window and creates the game window. The second, waitRunner, 
   * pauses the splash screen and then runs closeRunner. Finally, the last disposes the window.
   * 
   ...
   * @param firstRun - boolean - True if opened on the first run, false if opened as the about window.
   * 
   ...
   * @param labelSize - Dimension - The size of the splash screen.
  ...
  public SplashScreen(String filename, final int waitTime, boolean firstRun)
  {
    ...
    Dimension labelSize = label.getPreferredSize(); //returns size of splashscreen image
